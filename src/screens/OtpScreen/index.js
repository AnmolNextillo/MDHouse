...
const OtpScreen = ({ navigation, route }) => {
  ...
  const responseResendOtp = useSelector(
    (state) => state.resendOtpApiReducer.data,
  );
  ...
  const saveToken = async (loginData) => {
    ...
    if (userType == 3) {
      await AsyncStorage.setItem(
        "agentType",
        JSON.stringify(loginData.data.agentType),
      );
      ...
      if (loginData.data.studentType == 1) {
        ...
        if (loginData.data.isProfileCompleted == 1) {
          ...
          navigation.reset({
            index: 0,
            routes: [{ name: "AlumniDetails", params: { from: 1 } }],
          });
        }
      }
    }
  ...
  useEffect(() => {
    ...
    if (responseOtp != null) {
      if (responseOtp.status === 1) saveToken(responseOtp);
        ...
        Alert.alert(
          "MD House",
          responseOtp.message || "OTP verification failed. Please try again.",
        );
    }
  }, [responseOtp]);
  ...
  useEffect(() => {
    if (responseResendOtp != null) {
      if (responseResendOtp.status === 1) {
        Alert.alert(
          "MD House",
          responseResendOtp.message || "OTP resent successfully",
        );
      } else {
        Alert.alert(
          "MD House",
          responseResendOtp.message ||
